...
# v0.0.22 Xray test diagnostics and failed config preservation
#
# Features:
...
EOF2
  ...
  if ! xray run -test -config "$xray_target_tmp" >"$xray_test_log" 2>&1; then
...
main_menu(){
  ...
  while true; do
    echo
    echo "===== Xray Edge Manager v0.0.22 ====="
    echo "1. 首次部署向导，推荐"
    echo "2. 安装/升级基础依赖"
