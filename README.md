- **v0.0.53-bestcf-multi-domain**: 优选域名（`CFDomain_*`）节点数量**可配置**，默认仍为 1（完全向后兼容）。此前 `generate_bestcf_subscription_nodes` 的 `domain` 分支硬编码 `1 total 1` —— 所有机器的优选域名节点都取 `bestcf-domain.txt` 的**第一条**（`bestcf.030101.xyz`），该域名一旦解析异常/被墙，**全部机器的 CFDomain 节点同时失效**（单点故障）。现改为读取 `BESTCF_TOTAL_LIMIT`（钳制 1–25，非法值回退 1），并在菜单 11 新增模式 **3（3 个优选域名节点）** 供选择。数据源 `DustinWin/BestCF` 的 `bestcf-domain.txt` 实测有 **25 条**可用。
