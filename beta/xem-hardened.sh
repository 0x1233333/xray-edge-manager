...
# v0.0.31-hardening — long subscription token + origin firewall + non-root Xray
#
# Features:
...
validate_or_regen_token(){
  ...
  if [[ ! "$current" =~ ^[a-f0-9]{32,128}$ ]]; then
    if [[ -n "$current" ]]; then
      warn "${key} 格式非法（应为32-128位十六进制），已自动重新生成。旧值：$current"
    fi
...
apply_cf_origin_firewall(){
  load_state
  [[ "${PROTOCOLS:-0}" == *2* || "${PROTOCOLS:-0}" == *5* ]] || { warn "未启用 CDN 协议，跳过 Cloudflare 源站限制。"; return 0; }
  ...
  log "已限制 CDN 源站入口：仅 Cloudflare IP 可访问 TCP 80/${port}；UDP ${HY2_PORT:-443} 不受影响。"
}
...
configure_cf_origin_firewall_prompt(){
  load_state
  [[ "${PROTOCOLS:-0}" == *2* || "${PROTOCOLS:-0}" == *5* ]] || return 0
  if confirm "是否限制 CDN 源站 TCP 80/${CDN_PORT:-443} 只允许 Cloudflare 回源？推荐开启；不影响 HY2 UDP 443" "Y"; then
    enable_cf_origin_firewall
  else
    save_kv "$STATE_FILE" ENABLE_CF_ORIGIN_FIREWALL "0"
...
main_menu(){
  ...
  while true; do
    echo
    echo "===== Xray Edge Manager v0.0.31-hardening ====="
    echo "1. 首次部署向导，推荐"
    echo "2. 安装/升级基础依赖"
